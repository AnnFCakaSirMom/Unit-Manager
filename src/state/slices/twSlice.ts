...
export const twReducer = (state: AppState, action: AppAction): TWState => {
    switch (action.type) {
        ...
        case 'UPDATE_TW_SEASON': {
            const currentEventIds = action.payload.events.map((e: any) => e.id);
            const obsoleteEvents = state.twEvents.filter((e: any) => e.seasonId === action.payload.season.id && !currentEventIds.includes(e.id));
            const obsoleteEventIds = obsoleteEvents.map((e: any) => e.id);

            const updatedTwEvents = state.twEvents.filter(e => e.seasonId !== action.payload.season.id).concat(action.payload.events);

            return {
                twAttendance: state.twAttendance,
                twSeasons: state.twSeasons.map((s: any) => s.id === action.payload.season.id ? action.payload.season : s),
                twEvents: updatedTwEvents,
                twRecords: state.twRecords.filter((r: any) => !obsoleteEventIds.includes(r.eventId))
            };
        }
        ...
        case 'DELETE_TW_SEASON':
            return {
                twAttendance: state.twAttendance,
                twSeasons: state.twSeasons.filter((s: any) => s.id !== action.payload.seasonId),
                twEvents: state.twEvents.filter((e: any) => e.seasonId !== action.payload.seasonId),
                twRecords: state.twRecords.filter((r: any) => {
                    const event = state.twEvents.find((e: any) => e.id === r.eventId);
        ...
        case 'DELETE_TW_EVENT':
            return {
                twAttendance: state.twAttendance,
                twSeasons: state.twSeasons,
                twEvents: state.twEvents.filter((e: any) => e.id !== action.payload.eventId),
                twRecords: state.twRecords.filter((r: any) => r.eventId !== action.payload.eventId)
            };
        ...
        case 'UPDATE_TW_PLAYER_RECORD': {
            const existingRecordIndex = state.twRecords.findIndex((r: any) => r.eventId === action.payload.eventId && r.playerId === action.payload.playerId);
        ...
        case 'HYDRATE_TW_ATTENDANCE':
            return {
                ...state,
                twAttendance: action.payload,
                twSeasons: state.twSeasons,
                twEvents: state.twEvents,
                twRecords: state.twRecords
            };
