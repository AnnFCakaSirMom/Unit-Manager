...
*   **Trigger Function Lockdown:** Pure trigger functions (`enforce_role_on_insert()`, `update_profile_updated_at_from_units()`, `update_tw_import_list_updated_at()`) have `EXECUTE` fully revoked from `anon`/`authenticated`/`PUBLIC`, since PostgreSQL fires them via the trigger mechanism regardless of the invoking role's grants — leaving the privilege in place only expanded the PostgREST RPC attack surface with no functional benefit. All `SECURITY DEFINER` functions also pin `SET search_path = public` to prevent schema-hijacking.
...
*   **Empty-Response Hydration Guards**: A complementary, independent safeguard to the `isDirty` check above — `loadPlayers`, `loadGroups`, `loadTWImport`, and `loadTWData` in `useDatabaseSync.ts` all skip dispatching their hydration action entirely when the fetch resolves to a fully empty result. This exists because a transient RLS/JWT timing gap (or, for the `handleQuery`-based fetches, any swallowed network error) can make a query legitimately-but-incorrectly return zero rows without throwing. Without this guard, an empty response would hydrate over real local (non-dirty) data — and for `groups`/`twAttendance`, `useCloudSync`'s diff-based deletion would then read "everything missing from the new list" as "the user deleted everything" and issue real `DELETE` calls against Supabase. **This was not a theoretical concern**: a missing guard on `loadTWImport` caused a live `tw_import_list` wipe during a 2026-07 performance/verification session (see `PROJECT_STATUS.md` §30). The tradeoff (same one already accepted for `loadPlayers`, see §26) is that a *genuinely* fully-emptied list won't visually clear until the next full reload — legitimate full clears (e.g. "Clear TW Attendance & Groups") already bypass this path via their own explicit `dispatch(setGroups([]))` / `dispatch(clearTWAttendance())` calls, so they are unaffected.
...
*   **Realtime Deletion Propagation**: When a profile is deleted or merged in the database, the delta sync fetch (`loadSinglePlayer`) returns `null`. Instead of ignoring this, `useDatabaseSync` dispatches a `deletePlayer` action to instantly clear the ghost/zombie profile from Redux without requiring a browser reload. The same pattern applies to `tw_import_list`: when `loadSingleTWEntry`'s fetch returns `null` (e.g. a name dropped from the roster by a newer Raid Helper re-import), `deleteTWEntry` removes it from every other connected officer's Redux state instantly, instead of leaving it stale until a reload.
...
*   **Bounded-Parallel Sync**: `useCloudSync` batches its player/group/TW-attendance upserts and deletes through a small `mapWithConcurrency` runner (max 8 concurrent) instead of a sequential `await`-in-`for`-loop. This removes the N-round-trips-in-series cost of a multi-item edit (e.g. bulk unit changes, a TW import touching many groups) while still capping simultaneous load on Supabase's connection pool — a naive unbounded `Promise.all` was deliberately avoided for large bulk operations.
*   **Suspicious Full-Wipe Guard**: As a second, independent layer beneath the empty-hydration guards described above, `useCloudSync` itself refuses to execute a diff-based deletion if a previously-populated list (≥3 items) appears completely empty in a single sync tick — it skips the delete, surfaces a warning, and deliberately leaves its "previous state" ref pointed at the last known-good (non-empty) snapshot rather than collapsing it to empty, so a subsequent correct fetch can self-heal without any data having been deleted. This exists specifically to catch *any future, still-unknown* bug of the same shape as the `loadTWImport` incident, not just the ones already patched at the hydration layer. `playerService.upsertPlayer` applies the equivalent guard at the per-player unit level: before wiping all of a player's `profile_units` rows because their local unit lists are empty, it checks whether Supabase still holds a meaningful number of rows (≥3) for that profile and blocks the delete if so.
...
## 4. Build & Bundle Architecture

*   **Tailwind (Build-Time, Not CDN)**: Styling is compiled at build time via `tailwindcss` + `postcss` + `autoprefixer` (`tailwind.config.js`, `postcss.config.js`, entry `src/index.css` imported from `src/index.tsx`), producing a single purged, static CSS file. This replaced an earlier Tailwind CDN `<script>` (the full JIT compiler shipped to and run in the browser on every page load) — a meaningful, one-time bundle/load-time cost with no functional benefit over a build-time pipeline.
*   **Route-Level Code Splitting**: `App.tsx` renders one of several mutually-exclusive main views (`AdminPanel`, `ProfileMatcher`, `TWStatisticsView`, `TWAttendanceView`, `GroupView`, `PlayerUnitView`) plus `HelpManualModal`. All are loaded via `React.lazy()` behind a single `Suspense` boundary (with a shared `ViewLoadingFallback` spinner) instead of being statically imported into the initial bundle, since a given session typically only ever visits a subset of them. `vite.config.ts` additionally splits vendor code into `vendor-react` / `vendor-supabase` / `vendor-redux` chunks via `manualChunks`.
*   **Production Console Stripping**: `vite.config.ts` sets `esbuild.drop: ['console', 'debugger']` for production builds only (`mode === 'production'`) — development keeps all logging intact.
